...
PROPERTY_SOURCE(CfdGui::ViewProviderPostObject, Gui::ViewProviderDocumentObject)

ViewProviderPostObject::ViewProviderPostObject() : m_blockPropertyChanges(false)
{
...
ViewProviderPostObject::~ViewProviderPostObject()
{
    m_shapeHints->unref();
    m_coordinates->unref();
...
void ViewProviderPostObject::attach(App::DocumentObject *pcObj)
{
...
SoSeparator* ViewProviderPostObject::getFrontRoot(void) const 
{
    return m_colorRoot;
}


void ViewProviderPostObject::setDisplayMode(const char* ModeName)
{
...
std::vector<std::string> ViewProviderPostObject::getDisplayModes(void) const
{
...
void ViewProviderPostObject::update() 
{
    if(!setupPipeline())
        return;
...
void ViewProviderPostObject::updateProperties() 
{
...
void ViewProviderPostObject::update3D() 
{
    vtkPolyData* pd = m_currentAlgorithm->GetOutput();
...
void ViewProviderPostObject::WritePointData(vtkPoints* points, vtkDataArray* normals, vtkDataArray* tcoords) 
{
    Q_UNUSED(tcoords)
    double *p;
    int i;
...
void ViewProviderPostObject::WriteColorData(bool ResetColorBarRange) 
{
    if(!setupPipeline())
        return;
...
void ViewProviderPostObject::WriteTransparency() 
{
    float trans = float(Transparency.getValue()) / 100.;
    m_material->transparency.setValue(trans);
}

void ViewProviderPostObject::updateData(const App::Property* p) 
{
...
bool ViewProviderPostObject::setupPipeline() 
{
	Cfd::PostObject *obj = 
		dynamic_cast<Cfd::PostObject*>(getObject());

	if (!obj) return false;

	vtkSmartPointer<vtkDataSet> data = 
		vtkDataSet::SafeDownCast(obj->Data.getValue());
...
void ViewProviderPostObject::onChanged(const App::Property* prop) 
{
...
bool ViewProviderPostObject::doubleClicked(void) 
{
...
bool ViewProviderPostObject::setEdit(int ModNum) 
{
...
void ViewProviderPostObject::setupTaskDialog(TaskDlgPost* dlg) 
{
    dlg->appendBox(new TaskPostDisplay(this));
}

void ViewProviderPostObject::unsetEdit(int ModNum) 
{
...
void ViewProviderPostObject::OnChange(Base::Subject< int >& /*rCaller*/, int /*rcReason*/) 
{
